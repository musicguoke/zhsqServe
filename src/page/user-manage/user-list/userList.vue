...
export default {
    ...
    created() {
        ...
        if (this.$store.state.id) {
            this.isProduct = true;
            this._getRolesSingleList(this.$store.state.id);
            this.userForm.sysId = this.$store.state.id;
        } else {
            ...
            getUserSysAndRole(this.userForm.arId).then(res => {
                if (!this.isProduct) {
                    ...
                    for (let i in res.data) {
                        ...
                        if (res.data[i].sysId == this.$store.state.id) {
        ...
        addOrUpdateFun() {
            if (!this.isProduct) {
                ...
                if (this.isAdd) {
                    ...
                    this.sysAndGroupList.map(v => {
                        if (v.sysId == this.$store.state.id) {
                            this.userForm.sysId += this.$store.state.id + ",";
                            this.userForm.grId += this.userForm.grIdProduct + ",";
                        } else {
                            this.userForm.sysId += v.sysId + ",";
