<template>
    <div>
        ...
        <div class="tablePage">
            <Page :total="pageLength" @on-change="pageChange" show-total show-elevator ref="synPage"></Page>
        </div>
        ...
        <Modal v-model="importModal" title='导入周边热点' @on-ok="saveImport">
          <Form :model="importForm" label-position="left" :label-width="100" ref="file_syn_form">
              <FormItem label="导入类型">
              ...
              <FormItem label="选择文件">
                  <Upload :action="`${uploadUrl}/sys/hotAround/importFile.do`" with-credentials :before-upload="boforeUpload" :on-success="handleSuccessUpload" accept=".xls,.xlsx" ref="synImportUpload">
                  ...
                  <p>1、导入文件大小不超过2MB.</p>
                  <p>2、支持Microsoft Office Excel的xls和xlsx文件,模板<a :href="`${uploadUrl}/sys/hotAround/downloadImportedFile.do`">点此下载.</a></p>
              </div>
          </Form>
        </Modal>
    </div>
...
data() {
  ...
  methods:{
        ...
        onOk: () => {
            ...
            deleteHotspot(data).then(res=>{
              if(res.code == 20000){
                  ...
                  this._getHotspot(1)
                  this.$refs.synPage.currentPage = 1 
              }else{
                  this.$Message.error(res.message)
                }
            })
    ...
    openImportModal(){
        ...
        if(this.$refs.synImportUpload._data.fileList){
            this.$refs.synImportUpload._data.fileList = []
        }
    },
    boforeUpload(file) {
    ...
    saveImport(){
        if (this.importForm.type === '') {
            ...
            let formData = new FormData(this.$refs.file_syn_form)
            formData.append('type', this.importForm.type)
            formData.append('file', this.importForm.file)
            this._importHotspot(formData)
        }
    },
    //导入文件
    _importHotspot(data){
        importHotspot(data).then(res=>{
            if(res.code == 20000){
                this.$Message.success("添加成功")
                this._getHotspot(1)
                this.$refs.synPage.currentPage = 1 
