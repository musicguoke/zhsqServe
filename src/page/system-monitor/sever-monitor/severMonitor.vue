<template>
  <Content>
    ...
    <Card :style="{maxHeight:managerHeight}">
      <v-search v-if="userinfo.role!==3" :delete-show="false" :searchShow="false" :importShow="false" :disabled="selectedId.length <= 0" @on-delete="deleteMany" @on-build="severAddOpen" />
      <div class="tableSize">
        <el-table :data="severData" border style="width: 100%">
          ...
          <el-table-column label="操作" align="center">
            <template slot-scope="scope">
              <Button type="primary" @click="getSeverDetail(scope)" size="small" class="marginRight">查看</Button>
            </template>
          </el-table-column>
        </el-table>
      </div>
