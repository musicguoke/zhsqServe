<template>
    <div id="service-analysis">
        <div class="head">
            ...
            <ul class="clearfix" v-else-if="config.searchName">
                <li class="head-item" v-for="(item, index) in tabs" :key="item.title" @click="getSearchNameDayData(index, item.day)" :class="{active:index===num}">
                    {{item.title}}
                </li>
                <li class="date-box">
                    <span>自定义日期:</span>
                    <div class="block">
                        <DatePicker :value="days" format="yyyy-MM-dd" type="daterange" placement="bottom-end" placeholder="请选择开始结束时间" style="width: 200px" @on-change="getSearchNameDateData"></DatePicker>
                    </div>
                </li>
            </ul>
        ...
        <div class="main">
            ...
            <p class="current-time">{{currentTime}}
                ...
                <router-link to="/data-statistics" tag="span" class="link key-link">数据访问统计报表</router-link>
            </p>
            ...
            <table border="1" style="table-layout: fixed;" id="out-table">
                <thead>
                    <tr v-if="data.length > 0" class="three">
                        ...
                        <th v-if="type !== '1' && cur !== 2">用户名
                            <i class="icon-sort"></i>
                        </th>
                        <th v-if="type == 3">搜索关键词
                            <i class="icon-sort"></i>
                        </th>
                        <th v-if="type == 3 && dataObj !== null && config.visitCode">数据名称
                            <i class="icon-sort"></i>
                        </th>
                        <th v-if="type == 2 && departmentObj !== null">部门
                            <i class="icon-sort"></i>
                        </th>
                        <th v-if="type == 3 && departmentObj !== null">部门
                            <i class="icon-sort"></i>
                        </th>
                        <th>访问次数
                            <i class="icon-sort"></i>
                ...
                <tbody>
                    <tr v-for="(item, index) in data">
                        ...
                        <th v-if="type == 2 && departmentObj !== null">{{departmentObj.visitName}}</i>
                        </th>
                        <th v-if="type == 3 && departmentObj !== null">{{departmentObj.visitName}}</i>
                        </th>
                        <td v-if="type == 2 && dataObj !== null" class="hover" @click="getvisitobj(item.id, item.type, item.name)">{{item.total}}</i>
                        </td>
                        <td v-else-if="type == 3 && userObj !== null" class="hover" @click="getuserobj(item.id, item.type, item.name)">{{item.total}}</i>
                        </td>
            ...
            <div class="block pagination">
                <Page v-if="config.bId && config.userId" :total="total" :current.sync="currentPage" @on-change="handleBidandUserId"></Page>
                ...
                <Page v-else-if="config.searchName" :total="total" :current.sync="currentPage" @on-change="handleSearchName"></Page>
...
<script>
import { dayData, dateData, dayLogData, dateLogData, dayUserLogData, dateUserLogData, getDataCount, filterCommon, getUserGroupSearchByDate, getBranchSearchByDate, getWordCountByDate, getWordSearchCountByDate } from '@/api/service-data'
...
export default {
    ...
    methods: {
        ...
        search() {
            ...
            if (this.cur == 1) {
                this._getBranchSearchByDate(Object.assign({}, this.params, data))
            } else if (this.cur == 0) {
                this._getUserGroupSearchByDate(Object.assign({}, this.params, data))
            } else if (this.cur == 2) {
                this.config.search = this.searchContent
                this.config.searchName = this.searchContent
                this._getWordSearchCountByDate(Object.assign({}, this.params, data))
            }
        },
        getItemData(index) {
            this.cur = index || 0
            ...
            if (this.cur == 1) {
                ...
                this._getWordCountByDate(Object.assign({}, this.params, this.config, data))
            }
        },
        ...
        getSearchNameDayData(index, day) {
            this.num = index
            if (day == -1) {
                this.params.endDate = ''
                this.params.startDate = ''
        ...
        getServiceDetail(id, type, name) {
            ...
            if (mtype == 3) {
                ...
                if (this.config.searchName && !this.config.level) {
                    this._getDateUserLogData(Object.assign({}, this.params, this.config, data))
                } else if (this.config.searchName && this.config.level) {
                    this._getDateUserLogData(Object.assign({}, this.params, this.config, data))
                } else {
                    data = {
                        type: 2
                ...
                this.config.userId = id
                this._getWordCountByDate(Object.assign(this.params, data, this.config))
            } else if (mtype == 1) {
                const data = {
                    type: 3
                }
        ...
        handleUserId(value) {
            ...
            console.log(3)
            this.params.start = (val - 1) * 10
            if (this.config.userId && !this.config.searchName) {
                this._getWordCountByDate(Object.assign(this.params, this.config, data))
            } else if (this.config.userId && this.config.searchName) {
                this._getDateUserLogData(Object.assign(this.params, this.config, data))
            }
        },
        handleSearchName(value) {
            ...
            this.params.start = (val - 1) * 10
            if (this.config.level == 1 && !this.config.search) {
                this._getWordSearchCountByDate(Object.assign({}, this.params, this.config, data))
            } else if (this.config.level == 1 && this.config.search) {
                this._getWordSearchCountByDate(Object.assign({}, this.params, this.config, data))
            } else {
                this._getDateUserLogData(Object.assign(this.params, this.config, data))
            }
        ...
        _getWordCountByDate(data) {
            getWordCountByDate(data).then((res) => {
                this.userData = []
                this.data = res.data
                this.type = res.type
                this.total = res.total
        ...
        _getWordSearchCountByDate(data) {
            getWordSearchCountByDate(data).then((res) => {
                this.userData = []
                this.config.level = 1
                this.data = res.data
                this.type = res.type
